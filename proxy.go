...
func (proxy *Proxy) server() {
	...
	for {
		...
		proxy.Toxics.StartLink(name+"upstream", client, upstream, stream.Upstream, client.RemoteAddr())
		proxy.Toxics.StartLink(name+"downstream", upstream, client, stream.Downstream, client.RemoteAddr())
	}
}
